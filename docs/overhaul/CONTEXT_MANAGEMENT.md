...
Context is loaded via the `/brief` skill, which shells out to the `dialectic` CLI:

```
/brief resume <session_id>
```

This approach:
- Uses Claude Code's native skill system (no MCP servers)
- Outputs JSON from CLI, formatted by the skill
- Works with pre-submit hooks for automatic budget injection

See `design-doc-backend-context.md` Section VIII for full CLI documentation.

### On Session Resume

When resuming via `/brief resume <session_id>`:
...
```
/brief resume sess_1705847293
```

Loads full state, shows scratchpad, suggests next action.

The `/brief` skill:
1. Calls `dialectic session resume <id>` to get JSON
2. Formats the scratchpad for human readability
3. Includes suggested action based on session status
...
5. **Claude Code native** — Uses skills and hooks (no MCP complexity)
6. **Budget aware** — Pre-submit hook injects status every turn
7. **Auto-compact** — UI can inject compression commands on budget alerts

## Budget Thresholds

See `design-doc-backend-context.md` Section VI for full budget tier documentation.

| Threshold | Action |
|-----------|--------|
| < 70% | Normal operation |
| 70-84% | Auto-compress Tier 4 (silent) |
| 85-94% | Warn user, suggest `/brief compact` |
| 95%+ | Force compress, UI may auto-inject |

The pre-submit hook shows budget status at the start of every turn:
